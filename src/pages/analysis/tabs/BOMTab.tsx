 ...
 *
 * Accepts BOMData directly (no getMarket slug lookup — Böhmer doesn't use
 * the market bundle pattern).
 *
 * - Executive summary explaining BOM and its relevance to product launch
 * - Output type chip row (product variant selector)
 * - L4 subsystem rows with confidence badges and variant chips
 * - Expandable L3 module detail per subsystem
 * - Böhmer anchor highlighting (yellow border + badge)
 *
 * Adapted from Marquardt BOMTab for Böhmer Systemtechnik.
 */
...
/* ── Böhmer note callout ─────────────────────────────────────────────────── */
function BohmerNoteCallout({ note }: { note: string }) {
  if (!note) return null;
  return (
    <div
      style={{
...
/* ── Main tab component ──────────────────────────────────────────────────── */
export default function BOMTab({ bomData }: { bomData: BOMData }) {
  ...
  return (
    <div className="section">
      ...
      <div className="section__eyebrow">
        Bill of Materials · NAICS {bomData.naicsCode}
      </div>
      ...
      <ExecutiveSummary kicker="What you are reading" title="Bill of Materials Analysis">
        <p className="answer">
          ...
          module, and sub-system that makes up the finished product. Reading the BOM reveals{" "}
          <strong>where Böhmer's products sit within their physical product architecture</strong> —
          which subsystems define the cost structure, which modules are the competitive differentiators,
          and where the key design choices are made.
        </p>
        <p className="answer" style={{ marginTop: 12 }}>
          Each row below is a <strong>L4 subsystem</strong> (major functional block). Clicking a row expands the
          L3 module-level detail. The variant chips show competing technologies for that subsystem.
          Rows with a{" "}
          <span
            style={{
          ...
          badge are the subsystems or modules that define Böhmer's competitive differentiation.
        </p>
      </ExecutiveSummary>
      ...
      {bomData.dataPending ? (
        ...
        <>
          {/* ── Böhmer position note ── */}
          <BohmerNoteCallout note={bomData.sensorNote} />

          {/* ── Output type filter row ── */}
          {bomData.outputTypes.length > 0 && (
            <div style={{ marginBottom: 20 }}>
              <div className="bom-filter-label">Select a Product Variant to filter context:</div>
              <VariantChips
          ...
          <ExecutiveSummary kicker="How to read this table">
            <p className="answer">
              Each row is a top-level subsystem (L4 = major subsystem) of the product. The percentage
              shown next to the identifier is the approximate BOM cost share. The colored chips show
              competing technologies — <strong>green = high confidence data</strong>, amber = medium
              confidence, red = low confidence / analyst estimate. Click any row to see L3 module-level
              detail and the Böhmer competitive anchor position.
            </p>
          </ExecutiveSummary>
