 ...
 * Legend is rendered at the TOP.
 *
 * Adapted from Marquardt app for Böhmer Systemtechnik.
 * Böhmer anchor positions are highlighted with yellow (replacing Marquardt sensor positions).
 */
...
const C = {
  ...
  anchorGreen: "#6fd59b",
  anchorGreenBg: "rgba(111,213,155,0.12)",
  anchorOrange: "#e9c46a",
...
/* ── Böhmer anchor badge ─────────────────────────────────────────────────── */
function BohmerBadge({ label }: { label: string }) {
  return (
    <span
      style={{
        display: "inline-flex",
...
/* ── Legend (TOP of diagram) ────────────────────────────────────────────── */
function Legend({ hasAnchor }: { hasAnchor: boolean }) {
  return (
    <div
      style={{
        display: "flex",
...
function DetailPanel({
  ...
  if (detail.unit) {
    ...
    const bLabel = marquardtPositionLabel(unit);
    ...
    return (
      <div style={{ padding: "22px 22px" }}>
        ...
        <h3 style={{ fontSize: 18, fontWeight: 700, color: C.textPrimary, marginBottom: 8, lineHeight: 1.3, letterSpacing: "-0.01em" }}>
          {unit.name.replace(/—\s*BÖHMER\s+\w+\s+POSITION/gi, "").trim()}
          {bLabel && <BohmerBadge label={bLabel} />}
        </h3>
      ...
      <div style={{ padding: "22px 22px" }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {units.map((u) => {
              const bl = marquardtPositionLabel(u);
              return (
                <div
                  key={u.id}
                  style={{
                    padding: "8px 12px",
                    background: bl ? C.accentDim : "rgba(255,255,255,0.03)",
                    borderRadius: 6,
                    border: `1px solid ${bl ? "rgba(253,255,152,0.18)" : C.border}`,
                    display: "flex",
                    alignItems: "center",
                    gap: 8,
                    fontSize: 12,
                    color: bl ? C.accent : C.textSecondary,
                  }}
                >
                  <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: C.textTertiary, flexShrink: 0 }}>
                    {u.id}
                  </span>
                  <span style={{ flex: 1 }}>{u.name.replace(/—\s*BÖHMER\s+\w+\s+POSITION/gi, "").trim()}</span>
                  {bl && <BohmerBadge label={bl} />}
                </div>
              );
            })}
          </div>
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          paddingLeft: isUnitSelected || bLabel ? 29 : 32,
                          transition: "background 0.12s",
                        }}
                        onMouseEnter={(e) => {
                          if (!isUnitSelected)
                            (e.currentTarget as HTMLDivElement).style.background =
                              bLabel ? "rgba(253,255,152,0.05)" : C.rowHover;
                        }}
                        onMouseLeave={(e) => {
                          if (!isUnitSelected)
                            (e.currentTarget as HTMLDivElement).style.background = bLabel
                              ? "rgba(253,255,152,0.025)"
                              : "transparent";
                        }}
                      >
                        ...
                        <span
                          style={{
                            fontSize: 12.5,
                            fontWeight: bLabel ? 600 : 400,
                            color: bLabel ? C.textPrimary : C.textSecondary,
                            paddingLeft: 4,
                            overflow: "hidden",
                            textOverflow: "ellipsis",
                            whiteSpace: "nowrap",
