/**
 * BOMCategoryRow — one L4 row with label, confidence badge, and variant chips.
 *
 * Matches Figma design:
 *   - Left: collapse/expand caret, L4 identifier in mono font, row label, confidence badge
...
/* ── Böhmer anchor badge ──────────────────────────────────────────────────── */
function BohmerAnchorBadge({ note }: { note?: string | null }) {
  return (
    <span
      style={{
        display: "inline-flex",
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        ...
        {module.isMarquardtAnchor && <BohmerAnchorBadge note={module.sensorNote} />}
      </div>
        ...
        <div className="bom-inline-variants">
          {module.alternatives.map((alt) => (
            <VariantCard
              ...
              detail={module.sensorNote ?? undefined}
            />
          ))}
        </div>
      )}
...
export default function BOMCategoryRow({ subsystem, rowIndex }: BOMCategoryRowProps) {
  ...
  // Use the real L4 ID from the JSON (e.g. "L4-H01"); fall back to index only if missing.
      ...
      >
        ...
        <div className="bom-category-row__text-cell">
          <div className="bom-category-row__label-row">
            ...
            {subsystem.isMarquardtAnchor && <BohmerAnchorBadge />}
