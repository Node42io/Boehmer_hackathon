 ...
 *   4. Investment Priorities (ranked by ROI and timeline)
 *   5. Launch Sequence / Roadmap
 *   6. Succession Decision Framework
 */
...
const SCENARIO_STYLES: Record<string, { header: string; border: string }> = {
  A: { header: "#6fd59b", border: "rgba(111, 213, 155, 0.25)" },
  B: { header: "#d5a96f", border: "rgba(213, 169, 111, 0.25)" },
  C: { header: "#d56f6f", border: "rgba(213, 111, 111, 0.25)" },
};
...
const INVESTMENTS = [
  {
    rank: 1,
    action: "Qualify external galvanizer",
    cost: "EUR 2,000–5,000",
    timeline: "1–2 months",
    impact: "Unblocks both products (C3 constraint resolved)",
    priority: "CRITICAL PATH",
  },
  {
    rank: 2,
    action: "Heuraufe prototype — 3 units",
    cost: "EUR 5,000–8,000",
    timeline: "2–3 months",
    impact: "Validates production process + unit cost model",
    priority: "HIGH",
  },
  {
    rank: 3,
    action: "CE risk assessment — both products",
    cost: "EUR 3,000–6,000",
    timeline: "2–4 months",
    impact: "Regulatory clearance for market launch",
    priority: "HIGH",
  },
  {
    rank: 4,
    action: "Paddockbox hinge fatigue test (10,000 cycles)",
    cost: "EUR 4,000–8,000",
    timeline: "3–5 months",
    impact: "Validates critical design element; prerequisite for CE Machinery Directive file",
    priority: "HIGH",
  },
  {
    rank: 5,
    action: "Product landing page + inquiry form",
    cost: "EUR 1,000–3,000",
    timeline: "1–2 months",
    impact: "First lead generation; cost-efficient market signal before trade fair",
    priority: "MEDIUM",
  },
  {
    rank: 6,
    action: "Trade fair booth — regional equestrian fair",
    cost: "EUR 5,000–10,000",
    timeline: "6–9 months",
    impact: "First customer acquisition; pilot customer recruitment",
    priority: "MEDIUM",
  },
  {
    rank: 7,
    action: "Fachhandel partnership outreach (Kerbl/Loesdau)",
    cost: "EUR 0 (time investment)",
    timeline: "6–12 months",
    impact: "Scalable distribution channel without building sales force",
    priority: "MEDIUM",
  },
];
...
const SCENARIOS = [
  {
    id: "A",
    label: "Take Over & Transform",
    condition: "Viable IF: first 10 Heuraufe units sell within 9 months AND unit margin >30% AND Paddockbox prototype passes fatigue test.",
    description:
      "Launch products, gradually transition capacity from contract manufacturing to own-brand over 3 years. Build Fachhandel channel, attend trade fairs annually, develop adjacent product A4 (Prefab Stall System) as second revenue line. The company becomes an equestrian equipment OEM with contract manufacturing as the shrinking legacy base.",
    riskNote: "Primary risk: working capital. First 6 months require negative cash position (invest before revenue). Pilot production on Heuraufe validates the cost model before this scenario is confirmed.",
  },
  {
    id: "B",
    label: "Take Over & Manage Decline",
    condition: "Appropriate IF: products fail validation (margin <15%) OR market response is weak (Heuraufe units sold Y1 <5).",
    description:
      "Continue contract manufacturing. Optimize cash generation from existing machine park. Do not invest in product business. Focus on retaining highest-margin contract customers. Consider selective capacity reduction to improve utilization rate on remaining assets.",
    riskNote: "Structural risk: contract order book continues to decline as customers verticalize. This scenario buys time but does not reverse the trend. Exit horizon: 5–10 years.",
  },
  {
    id: "C",
    label: "Sell",
    condition: "Appropriate IF: no viable product path confirmed AND contract business terminal (order book decline >20%/yr).",
    description:
      "Find buyer for machine park (laser cutter, press brake, welding stations) and customer relationships. DIN EN ISO 3834-2 / EN 1090 certifications may carry value for a buyer. Potential acquirers: larger regional fabricator seeking capacity, private equity roll-up of Mittelstand metal shops.",
    riskNote: "Asset value is the certified machine park + certifications. Customer relationships are uncontracted and may not transfer.",
  },
];

const TRIGGER_METRICS = [
  {
    metric: "Heuraufe units sold (Year 1)",
    transform: "> 20 units",
    decline: "< 5 units",
    transformColor: "#6fd59b",
    declineColor: "#d56f6f",
  },
  {
    metric: "Unit margin (Heuraufe)",
    transform: "> 30%",
    decline: "< 15%",
    transformColor: "#6fd59b",
    declineColor: "#d56f6f",
  ...
  {
    metric: "Paddockbox hinge fatigue test",
    transform: "PASS",
    decline: "FAIL",
    transformColor: "#6fd59b",
    declineColor: "#d56f6f",
  ...
  {
    metric: "Customer reorder rate (Y2)",
    transform: "> 40%",
    decline: "< 10%",
    transformColor: "#6fd59b",
    declineColor: "#d56f6f",
  ...
  {
    metric: "Galvanizer qualified",
    transform: "Month 1–2",
    decline: "Delayed > 6 months",
    transformColor: "#6fd59b",
    declineColor: "#d5a96f",
  },
];
...
function PriorityBadge({ priority }: { priority: string }) {
  const color = PRIORITY_COLORS[priority] ?? "var(--text-gray)";
  return (
    <span style={{
...
export default function StrategicSynthesis() {
  ...
  return (
    <>
      ...
      <section id="syn-investment" className="container" style={{ paddingTop: 48 }}>
        <SectionAnchor
          id="syn-investment"
          title="Investment Priorities"
          kicker="Ranked by ROI × timeline"
        />
        <div className="md">
          <p style={{ marginBottom: 20 }}>
            Total Phase 1 investment: <strong style={{ color: "var(--accent-yellow)" }}>EUR 20,000–43,000</strong>.
            Priority ordering reflects dependency structure, not preference — CRITICAL PATH items
            must complete before HIGH items can start.
          </p>
        </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{
                ...
                {["#", "Investment", "Est. Cost (EUR)", "Timeline", "Impact", "Priority"].map((h) => (
                  <th key={h} style={{
                    padding: "12px 16px",
                    textAlign: "left",
                    fontFamily: "var(--font-mono)",
            ...
            <tbody>
              {INVESTMENTS.map((inv, i) => (
                <tr
                  key={inv.rank}
                  style={{
                    borderBottom: i < INVESTMENTS.length - 1 ? "1px solid var(--divider)" : undefined,
                    background: inv.priority === "CRITICAL PATH"
                      ? "rgba(213,111,111,0.025)"
                      : "transparent",
                  }}
                >
                  <td style={{
                    ...
                    fontSize: 13,
                    fontWeight: 700,
                    color: PRIORITY_COLORS[inv.priority] ?? "var(--text-gray)",
                    textAlign: "center",
                    width: 36,
                  }}>
                    {inv.rank}
                  </td>
                  <td style={{ padding: "13px 16px", color: "var(--text-white)", fontWeight: 500, maxWidth: 260 }}>
                    {inv.action}
                  </td>
                  <td style={{
                    padding: "13px 16px",
        ...
        <div style={{
          ...
          {[
            { label: "Phase 1 (Months 1–6)",  value: "EUR 20,000–43,000", sub: "Galvanizer + Prototype + CE + Landing page" },
            { label: "Phase 2 (Months 6–12)", value: "EUR 15,000–25,000", sub: "Trade fair + Paddockbox CE + Pilot production" },
            { label: "Phase 3 (Months 12–18)",value: "EUR 20,000–40,000", sub: "Paddockbox launch + Fachhandel development" },
          ].map((item) => (
      ...
      {/* ── 6. Succession Decision Framework ────────────────────── */}
      <section id="syn-succession" className="container" style={{ paddingTop: 48 }}>
        <SectionAnchor
          id="syn-succession"
          title="Succession Decision Framework"
          kicker="Decision support — not a recommendation"
        />

        <div className="md">
          <p style={{ marginBottom: 20 }}>
            This section is framed as decision support for Matthias Böhmer's succession decision —
            take over &amp; transform, take over &amp; manage decline, or sell. The three scenarios
            are mutually exclusive but not permanently so: Scenario A must be confirmed or rejected
            within 9–12 months based on the trigger metrics below. The product analysis has already
            eliminated the "skip product launch" variant of Scenario A — there is no viable
            transformation without own products.
          </p>
        </div>
            ...
            return (
              <div key={sc.id} style={{
                ...
                <div style={{
                  ...
                  <p style={{ fontSize: 11.5, color: s.header, margin: 0, lineHeight: 1.55, fontStyle: "italic" }}>
                    {sc.condition}
                  </p>
                </div>
                ...
                <div style={{ padding: "12px 18px" }}>
                  ...
                  <p style={{ fontSize: 11, color: "var(--text-gray)", margin: 0, lineHeight: 1.55 }}>
                    {sc.riskNote}
                  </p>
                </div>
              </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{
                  ...
                  {["Metric", "Transform threshold (→ Scenario A)", "Decline threshold (→ Scenario B/C)"].map((h) => (
                    <th key={h} style={{
                      padding: "12px 18px",
                      textAlign: "left",
                      fontFamily: "var(--font-mono)",
              ...
              <tbody>
                {TRIGGER_METRICS.map((row, i) => (
                  <tr
                    key={row.metric}
                    style={{
                      borderBottom: i < TRIGGER_METRICS.length - 1 ? "1px solid var(--divider)" : undefined,
                    }}
                  >
                    <td style={{
                      padding: "13px 18px",
                    ...
                    <td style={{ padding: "13px 18px" }}>
                      <span style={{
                        ...
                        fontSize: 12,
                        fontWeight: 600,
                        color: row.transformColor,
                        background: `${row.transformColor}18`,
                        padding: "3px 9px",
                        borderRadius: 5,
                      }}>
                        {row.transform}
                    ...
                    <td style={{ padding: "13px 18px" }}>
                      <span style={{
                        ...
                        fontSize: 12,
                        fontWeight: 600,
                        color: row.declineColor,
                        background: `${row.declineColor}18`,
                        padding: "3px 9px",
                        borderRadius: 5,
                      }}>
                        {row.decline}
        ...
        <div style={{
          ...
          <p style={{ fontSize: 12.5, color: "var(--text-gray-light)", margin: 0, lineHeight: 1.65 }}>
            This analysis covers the product viability question — capability fit, market needs, constraints, competitive positioning, and JTBD outcomes. It does not model the interaction between the new product business and the existing contract manufacturing order book. That capacity allocation analysis (Phase 2 per Briefing2.md) is a separate workstream requiring actual machine utilization data and contract order pipeline visibility. The succession decision should incorporate both before reaching a final conclusion.
          </p>
        </div>
      </section>
    </>
